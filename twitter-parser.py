...
while True:
    queue_cursor.execute("select data, sid, id from twitter_statuses"+table_suffix+" where sid > "+str(max_id)+" limit 500")
    res = queue_cursor.fetchone()
    if res==None:
        if table_suffix != tableSuffix():
            queue_cursor.execute("truncate table twitter_statuses"+table_suffix)
            table_suffix = tableSuffix()
            max_id = 0
    while res:
        lock.acquire()
